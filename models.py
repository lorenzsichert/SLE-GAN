...
class UpBlock(nn.Module):
    def __init__(self, in_planes, out_planes, size, z_dim, class_emb_dim, seperable=False):
        super().__init__()
        self.up = nn.Upsample(scale_factor=2, mode='nearest')
        self.conv = nn.utils.spectral_norm(nn.Conv2d(in_planes, out_planes*2, 3, 1, 1, bias=False))
        if seperable == True:
            self.conv = nn.Sequential(
                nn.utils.spectral_norm(nn.Conv2d(in_planes, in_planes, 3, 1, 1, groups=in_planes)),
                nn.utils.spectral_norm(nn.Conv2d(in_planes, out_planes*2, 1, 1, 1, bias=False)),
            )
